$(document).bind("mobileinit", function(){
    $.mobile.notesdb = openDatabase("AnPingFood", "1.0", "AnPing Food Map", 2*1024*1024);
    $.mobile.notesdb.transaction(function (t) {
	    t.executeSql("CREATE TABLE IF NOT EXISTS Food (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL,details TEXT NOT NULL, created TEXT NOT NULL, updated TEXT, latitude REAL, longitude REAL, pic TEXT);");
		//
	  ...
	  //t.executeSql('DROP TABLE Food');
		...
		if (isLoadData==null){  // 第一次載入預設的資料
			...
			$.each(FoodData, function(InfoIndex, Info) {
				...
				var pic = Info["pic"];
				t.executeSql('INSERT into Food(title, details, created, latitude, longitude, pic) VALUES (?,?,date("now"),?,?,?);',[title, details, lat, lng, pic]);
		    })
		}
    });
});
...
if (FoodNote.lat==null || FoodNote.lng==null ){
	FoodNote.lat=22.998772;  // 開始時，以文章牛肉湯為中心點
	FoodNote.lng=120.169731;
							...
							if (result2.rows.length>0){
								for (j= 0; j < result2.rows.length; j += 1) {
									 ...
									 if (disp<0.001) {
										 ...
										 // 下列這一行為新加入，用以強迫結束兩層的 for 迴圈
										 j=	result2.rows.length+1;i=len+1; // 強迫結束地標上加入
...
function getTitles() {
	...
	var items = [];
	$.mobile.notesdb.transaction(function(t) {
		t.executeSql("SELECT id, title,details,latitude,longitude,pic FROM Food ORDER BY id DESC LIMIT ?", [FoodNote.limit], function(t, result) {
...
function getItem(id) {
	$.mobile.notesdb.transaction(function(t) {
		t.executeSql("SELECT * FROM Food WHERE id = ?", [id], function(t, result) {
			...
			$("#showpic").html("<img src='"+row.pic+"' width='210' height='210' id='pics'>");
